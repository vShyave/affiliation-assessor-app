...
const BASE_URL =
  process.env.WEB_PORTAL_HASURA_API_URL || "https://hasura.upsmfac.org/api/";
...
adminCustomPost.interceptors.request.use(
  (request) => {
    ...
    request.headers["x-hasura-admin-secret"] = X_HASURA_ADMIN_SECRET_KEY; // request.headers['Authorization'] = `Bearer ${user_data.token}`;
